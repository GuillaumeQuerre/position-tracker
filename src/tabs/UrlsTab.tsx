...
type SortColUrl = 'url' | 'position' | 'kwCount'
type SortDir = 'asc' | 'desc'

function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  return <span style={{ marginLeft: 4, fontSize: 9, opacity: active ? 1 : 0.3, color: active ? '#a3f1eb' : '#4a7a7a' }}>{active ? (dir === 'asc' ? '↑' : '↓') : '↕'}</span>
}
...
export function UrlsTab() {
  ...
  const sortCol = tabPrefs.urlSortCol ?? 'kwCount'
  const sortDir = tabPrefs.urlSortDir ?? 'desc'

  function toggleSort(col: SortColUrl) {
    const newDir = sortCol === col ? (sortDir === 'asc' ? 'desc' : 'asc') : (col === 'url' ? 'asc' : 'desc')
    setTabPrefs({ urlSortCol: col, urlSortDir: newDir })
  }

  const filtered = useMemo(() => {
    let list = urls.filter(u => {
      const matchSearch = !search || u.url.toLowerCase().includes(search.toLowerCase())
      const matchNoTag = !filterNoTag || u.tags.length === 0
      return matchSearch && matchNoTag
    })
    list.sort((a, b) => {
      if (sortCol === 'url') return sortDir === 'asc' ? a.url.localeCompare(b.url) : b.url.localeCompare(a.url)
      if (sortCol === 'position') {
        const pa = a.latestPosition ?? 999, pb = b.latestPosition ?? 999
        return sortDir === 'asc' ? pa - pb : pb - pa
      }
      return sortDir === 'asc' ? a.keywordCount - b.keywordCount : b.keywordCount - a.keywordCount
    })
    return list
  }, [urls, search, filterNoTag, sortCol, sortDir])
  const allSelected = filtered.length > 0 && filtered.every(u => selected.includes(u.id))
  const noTagCount = urls.filter(u => u.tags.length === 0).length
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        {noTagCount > 0 && (
          <button onClick={() => setFilterNoTag(f => !f)}
            className="px-3 py-2 rounded-lg text-xs font-medium border transition-all whitespace-nowrap"
            style={filterNoTag ? {background: C.primary, borderColor: C.primary, color: C.bg} : {background: C.surface, borderColor: C.border, color: C.muted}}>
            {noTagCount} sans tag
          </button>
        )}
        ...
        <AISegmentButton mode="urls" onDone={() => window.location.reload()} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{borderBottom: `1px solid ${C.border}`, background: C.bg}}>
              ...
              <th onClick={() => toggleSort('url')} className="py-3 px-3 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none" style={{color: sortCol === 'url' ? C.light : C.primary}}>
                URL<SortIcon active={sortCol === 'url'} dir={sortDir} />
              </th>
              <th onClick={() => toggleSort('position')} className="py-3 px-3 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none" style={{color: sortCol === 'position' ? C.light : C.primary}}>
                Position<SortIcon active={sortCol === 'position'} dir={sortDir} />
              </th>
              <th onClick={() => toggleSort('kwCount')} className="py-3 px-3 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none w-16" style={{color: sortCol === 'kwCount' ? C.light : C.primary}}>
                Mots-clés<SortIcon active={sortCol === 'kwCount'} dir={sortDir} />
              </th>
